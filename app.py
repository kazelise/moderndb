...
DEFAULT_AI_URL = "https://openrouter.ai/api/v1"  # Default URL for the AI service
DEFAULT_API_KEY = ""  # Default API Key for the AI service
...
def append_terminal_output(msg, max_lines=5):
...
# Retrieves AI configuration (URL and API key). Currently uses hardcoded defaults.
def get_ai_config():
    ai_url = DEFAULT_AI_URL
    api_key = DEFAULT_API_KEY
    return ai_url, api_key
...
def check_ai_status():
    ai_url, api_key = get_ai_config()
    model_name = "qwen/qwen3-235b-a22b:free"  # Hardcoded model name
    if ai_url and api_key:
        append_terminal_output(f"<span style='color:green;'>AI Status: Model '{model_name}' is configured and ready.</span>")
    else:
        append_terminal_output("<span style='color:red;'>AI Status: AI service is not configured.</span>")
    return redirect(url_for("index"))
...
def ai_command():
    ...
    ai_url = DEFAULT_AI_URL
    api_key = DEFAULT_API_KEY
    model = "qwen/qwen3-235b-a22b:free"  # Hardcoded AI model

    if not ai_url:  # API key check is implicitly handled by AI service if URL is present
        append_terminal_output("Please configure AI service first.")
        return redirect(url_for("index"))
